...
contract DysonToGo is IERC721Receiver {
    ...
    function bribeClaimRewards(
        address gauge,
        address token,
        uint[] calldata week
    ) external onlyOwner returns (uint amount) {
        address bribe = AddressBook(ADDRESS_BOOK).bribeOfGauge(gauge);
        if (week.length == 1)
            amount = IBribe(bribe).claimReward(token, week[0]);
        else amount = IBribe(bribe).claimRewards(token, week);
        token.safeTransfer(owner, amount);
    }

    function bribeClaimRewardsMultipleTokens(
        address gauge,
    ...
    function deposit(
        ...
        uint minOutput,
        uint time
    ) external lock returns (uint output) {
    ...
    function depositETH(
        ...
        uint minOutput,
        uint time
    ) external payable lock returns (uint output) {
        uint spBefore = _update();
    ...
    function _claimDyson(
        address to,
        uint sp
    ) internal returns (uint dysonAmount) {
        if (sp == 0) return 0;
        spSnapshot = _update();
    ...
    function withdraw(
        ...
        uint index,
        address to
    ) external lock returns (uint token0Amt, uint token1Amt, uint dysonAmt) {
    ...
    function withdrawETH(
        ...
        require(position.hasDepositedAsset, "not deposited");
        position.hasDepositedAsset = false;
        (token0Amt, token1Amt) = IPair(pair).withdraw(
            position.index,
            address(this)
        );
        address token = token0Amt > 0
            ? IPair(pair).token0()
            : IPair(pair).token1();
...
contract DysonToGoFactory {
    ...
    function createDysonToGo(
        address user
    ) external returns (address dysonToGo) {
